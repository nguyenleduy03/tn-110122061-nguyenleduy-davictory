...
#
#   Docker: dùng ./docker.sh thay vì manage.sh
# ===============================================================
set -uo pipefail
...
AI_AGENT_DIR="ai-agent-python"; AI_IMPORT_DIR="ai-import-python"
...
status() {
  echo ""
  echo -e "  ${BOLD}Mode: Production (Native Process)${NC}"
  echo ""
  printf "  ${BOLD}%-20s %8s %s${NC}\n" "SERVICE" "PORT" "STATUS"
  echo "  $(printf '─%.0s' {1..55})"
  local count=0 total=0
  for svc in backend frontend ai-writing ai-speaking ai-agent ai-import chroma; do
    total=$((total + 1))
    local port; port=$(_port "$svc") || port="-"
...
menu() {
  while true; do
    ...
    echo ""
    echo -e "${BOLD}╔══════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║     DAVictory Production Manager        ║${NC}"
    echo -e "${BOLD}╚══════════════════════════════════════════╝${NC}"
    echo ""
    status
    echo -e "  ${BOLD}── MENU ──${NC}"
    ...
    case "$c" in
      ...
      3)
        ...
        for s in backend frontend ai-writing ai-speaking ai-agent ai-import chroma; do start_svc "$s"; done
        _ok "Tất cả services đã khởi động lại"
        ;;
      4) build_frontend ;;
      5) build_backend ;;
      6) build_ai_frontend ;;
...
if [[ $# -eq 0 ]]; then
  ...
  case $action in
    ...
    *)
      ...
      echo ""
      echo "  Services: backend frontend ai-writing ai-speaking ai-agent ai-import chroma"
      echo "  Docker:   dùng ./docker.sh thay vì manage.sh"
      exit 1
      ;;
  esac
fi